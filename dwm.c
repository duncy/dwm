...
enum { NetSupported, NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation, NetSystemTrayVisual, 
       ...
       NetSystemTrayOrientationHorz, NetWMWindowTypeDialog, NetClientList, NetDesktopNames, 
       NetDesktopViewport, NetNumberOfDesktops, NetCurrentDesktop, NetWMDesktop, NetWMCheck, NetLast }; /* EWMH atoms */
...
arrangemon(Monitor *m)
{
	...
	XMoveResizeWindow(dpy, selmon->tabwin, selmon->wx + sp, selmon->ty, selmon->ww - 2 * sp, th);//tab
...
createmon(void)
{
	...
	m->tagmap = ecalloc(NUMTAGS, sizeof(Pixmap));
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		m->tabwin = XCreateWindow(dpy, root, m->wx + sp, m->ty, m->ww - 2 * sp, th, 0, depth,
                                  InputOutput, visual,
						          CWOverrideRedirect|CWBackPixel|CWBorderPixel|CWColormap|CWEventMask, &wa);
...
updatebarpos(Monitor *m)
{
	...
	if (m->showbar) {
		...
		if ( m->topbar ) m->wy = m->wy + bh + vp;//tab
	} else {//tab
		m->by = -bh - vp;
	}
